...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, df: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
    ...
    if df is not None:
        ...
        for col in df.columns:
            ...
            is_object_type = df[col].dtype == 'object'
            is_string_type = isinstance(df[col].dtype, pd.StringDtype)
            is_categorical_type = isinstance(df[col].dtype, pd.CategoricalDtype)
            
            if is_object_type or is_string_type or is_categorical_type:
